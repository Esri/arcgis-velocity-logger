...
(function testArtifactSigningPlanUsesOnlyBuiltSignableFiles() {
  const plan = getArtifactSigningPlan({
    artifactPaths: [
      path.join('/repo/dist', 'arcgis-velocity-logger-1.0.2-setup.exe'),
      path.join('/repo/dist', 'arcgis-velocity-logger-1.0.2-portable.exe'),
      path.join('/repo/dist', 'arcgis-velocity-logger-1.0.2-win.zip'),
      path.join('/repo/dist', 'arcgis-velocity-logger-1.0.2-linux.AppImage'),
    ],
  });

  assert.deepStrictEqual(plan.sourceDirs, ['/repo/dist']);
  assert.strictEqual(plan.fileMask, 'arcgis-velocity-logger-1.0.2-setup.exe;arcgis-velocity-logger-1.0.2-portable.exe');
})();
