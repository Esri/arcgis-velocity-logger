...
(function testParseSignOptions() {
  const parsed = parseSignOptions([
    ...
    '--sign-product-names', 'ArcGIS Velocity Logger,ArcGIS Velocity Simulator',
  ...
  assert.strictEqual(parsed.signProductNames, 'ArcGIS Velocity Logger,ArcGIS Velocity Simulator');
})();
...
(function testBuildSignEnvPreservesReleaseProvidedSigningEnv() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'velocity-logger-sign-env-test-'));
  try {
...
(function testExternalSigningAddsPathAwareWindowsSignHook() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'velocity-logger-sign-config-test-'));
  try {
...
(function testExternalSigningReplacesConflictingSigningOverrides() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'velocity-logger-sign-config-conflict-test-'));
  try {
    const scriptFile = path.join(tempDir, 'sign.sh');
    fs.writeFileSync(scriptFile, '#!/usr/bin/env bash\nexit 0\n');
...
(function testWindowsSignHookSkipsOnlyDirectExternalSignSourceFiles() {
  const projectDir = path.join('/repo', 'velocity-logger');

  assert.strictEqual(isDirectExternalSignableFile(path.join(projectDir, 'dist', 'arcgis-velocity-logger-1.0.3-setup.exe'), projectDir), true);
  assert.strictEqual(isDirectExternalSignableFile(path.join(projectDir, 'dist', 'arcgis-velocity-logger-1.0.3-portable.exe'), projectDir), true);
  ...
  assert.strictEqual(isDirectExternalSignableFile(path.join(projectDir, 'dist', 'win-unpacked', 'VelocityLogger.exe'), projectDir), true);
  ...
  assert.strictEqual(isDirectExternalSignableFile(path.join(projectDir, 'dist', 'arcgis-velocity-logger-1.0.3-win.zip'), projectDir), false);
})();

(function testPackageUsesAfterSignHookForUnpackedExternalSigning() {
  const pkg = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'package.json'), 'utf8'));

  assert.strictEqual(pkg.build.productName, 'ArcGIS Velocity Logger');
  assert.strictEqual(pkg.build.executableName, 'VelocityLogger');
...
(function testBuildSignCommandDefaultsToRun() {
  const command = buildSignCommand({
    ...
    productNames: ['ArcGIS Velocity Logger'],
  ...
  assert.deepStrictEqual(command.args, [
    ...
    '--product-names', 'ArcGIS Velocity Logger',
...
(function testTargetCleanupRemovesOnlyRequestedPlatformArtifacts() {
  const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'velocity-logger-artifact-clean-test-'));
  try {
    ...
    fs.writeFileSync(path.join(distDir, 'arcgis-velocity-logger-1.1.0-setup.exe'), 'old');
    fs.writeFileSync(path.join(distDir, 'arcgis-velocity-logger-1.0.5-portable.exe'), 'current');
    fs.writeFileSync(path.join(distDir, 'arcgis-velocity-logger-1.0.5-mac.zip'), 'mac');

    const removed = cleanTargetArtifacts(repoDir, ['--win'], () => {});
    assert.strictEqual(removed.length, 2);
    assert.strictEqual(fs.existsSync(path.join(distDir, 'arcgis-velocity-logger-1.0.5-mac.zip')), true);
  } finally {
...
(function testExistingArtifactSigningPlanAndSignableFileDetection() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'velocity-logger-sign-test-'));
  try {
    fs.writeFileSync(path.join(tempDir, 'logger-setup.exe'), 'fake');
    fs.writeFileSync(path.join(tempDir, 'logger-win.zip'), 'fake');
    ...
    assert.strictEqual(plan.fileMask, 'logger-setup.exe');
  } finally {
    fs.rmSync(tempDir, { recursive: true, force: true });
  }
})();
...
(function testOfficialProductName() {
  assert.strictEqual(getOfficialProductName({}), 'ArcGIS Velocity Logger');
})();

(function testExternalSignLockSerializesConcurrentProcesses() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'velocity-logger-sign-lock-test-'));
  try {
...
async function testRunExternalSignDoesNotWaitForStdin() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'velocity-logger-sign-stdin-test-'));
  ...
  try {
    const sourceFile = path.join(tempDir, 'logger.exe');
...
`);
    ...
    await externalSign({
      ...
      packager: { appInfo: { productName: 'ArcGIS Velocity Logger' } },
    });
...
async function testWindowsSignHookSkipsDirectDistAndDelegatesNestedFiles() {
  const projectDir = path.join('/repo', 'velocity-logger');
  const directArtifact = path.join(projectDir, 'dist', 'arcgis-velocity-logger-1.0.3-setup.exe');
  const directUnpackedExe = path.join(projectDir, 'dist', 'win-unpacked', 'VelocityLogger.exe');
