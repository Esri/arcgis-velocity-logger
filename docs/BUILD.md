...
|---------------|----------------------------------|--------------------------------------|--------------|
| Windows unpacked app (`afterPack`) | `dist/win-unpacked` | `ArcGIS Velocity Logger` | Top-level files matching `*.exe;*.msi;*.msp` in `win-unpacked` (normally `ArcGIS Velocity Logger.exe`). |
