...
The packaged Windows app metadata uses the official product name **ArcGIS Velocity Logger**. This is the value written to the Windows executable **Product name** and **File description** fields. Release artifact filenames intentionally keep the slug prefix `arcgis-velocity-logger` for stable, script-friendly downloads.
...
**Executable/app bundle name:** `package.json → build.executableName` is set to `VelocityLogger`, so unpacked app outputs use space-free names such as `VelocityLogger.exe` on Windows and `VelocityLogger.app` on macOS while `productName` remains `ArcGIS Velocity Logger` for display metadata.
...
| `--sign-product-names <names>` | Optional | Passed to `sign.sh` as `--product-names <names>`. Defaults to the official app product name. Use colon-separated names for multiple source directories. |
...
|---------------|----------------------------------|--------------------------------------|--------------|
| Windows unpacked app (`afterSign`) | `dist/win-unpacked` | `ArcGIS Velocity Logger` | Top-level files matching `*.exe;*.msi;*.msp` in `win-unpacked` (normally `VelocityLogger.exe`) after electron-builder has finished Windows resource editing. The path-aware hook skips electron-builder/signtool signing for those direct files because this phase signs them externally. |
| Final Windows artifacts (`afterAllArtifactBuild`) | The final artifact folder, normally `dist` | `ArcGIS Velocity Logger` | Only the generated signable final artifacts from the current build, using an exact file mask such as `arcgis-velocity-logger-<version>-setup.exe;arcgis-velocity-logger-<version>-portable.exe`. ZIP files are not signed directly. |
...
bash /absolute/path/to/sign.sh --run \
  --timeout-minutes 20 \
  --source-dirs /Users/hano4470/github/Esri/arcgis-velocity-logger/dist/win-unpacked \
