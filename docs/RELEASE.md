...
| `--sign-product-names <names>` | Optional external signing product names passed to `sign.sh` as `--product-names <names>`. Defaults to `ArcGIS Velocity Logger`; use colon-separated names for multiple source directories. |
