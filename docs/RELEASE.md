...
|------|--------------------------|-----------------------------|------------------------|
| Logger | `ArcGIS Velocity Logger` | `/Users/hano4470/github/Esri/arcgis-velocity-logger/dist/win-unpacked` | The hook signs only current generated `*.exe`, `*.msi`, and `*.msp` final artifacts in `dist/` via an exact `--file-mask` value. |

The unpacked phase signs top-level `*.exe;*.msi;*.msp` files in `dist/win-unpacked` (normally `ArcGIS Velocity Logger.exe`). Final `.zip`, `.dmg`, `.deb`, and `.AppImage` artifacts are not directly signed by this Windows signing script.
